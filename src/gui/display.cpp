	...
	namespace gui {
...
#ifndef NDEBUG
			if(!m_initialized) {
				THROW_NESCC_GUI_DISPLAY_EXCEPTION(NESCC_GUI_DISPLAY_EXCEPTION_UNINITIALIZED);
			}
#endif // NDEBUG
		...
		display::create_texture(void)
		{
			...
			if(SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, m_crt ? DISPLAY_QUALITY_CRT : DISPLAY_QUALITY) == SDL_FALSE) {
				THROW_NESCC_GUI_DISPLAY_EXCEPTION_FORMAT(NESCC_GUI_DISPLAY_EXCEPTION_EXTERNAL,
			...
			) const
		{
			...
			if((x > DISPLAY_WIDTH) || (y > DISPLAY_HEIGHT)) {
				THROW_NESCC_GUI_DISPLAY_EXCEPTION_FORMAT(NESCC_GUI_DISPLAY_EXCEPTION_POSITION,
					"Position={%u, %u}", x, y);
			}
			...
			) const
		{
			...
			result << NESCC_GUI_DISPLAY_HEADER << "(" << SCALAR_AS_HEX(uintptr_t, this) << ")";

			if(verbose) {
				result << " Base=" << nescc::core::singleton<nescc::gui::display>::to_string(verbose);
