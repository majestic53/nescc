	...
	namespace emulator {
...
#ifndef NDEBUG
			if(!m_initialized) {
				THROW_NESCC_EMULATOR_CPU_EXCEPTION(NESCC_EMULATOR_CPU_EXCEPTION_UNINITIALIZED);
			}
#endif // NDEBUG
			...
			) const
		{
			...
			for(; iter < offset; ++iter) {
				...
				switch(command.second) {
					case CPU_MODE_ABSOLUTE:
						target = bus.cpu_read(program_counter++);
						target |= (bus.cpu_read(program_counter++) << CHAR_BIT);
						stream << "[" << SCALAR_AS_HEX(uint16_t, target)
							<< "]=" << SCALAR_AS_HEX(uint8_t, bus.cpu_read(target));
						break;
					case CPU_MODE_ABSOLUTE_X:
						target = bus.cpu_read(program_counter++);
						target |= (bus.cpu_read(program_counter++) << CHAR_BIT);
						boundary = (((target & UINT8_MAX) + m_index_x) > UINT8_MAX);
						target += m_index_x;
						stream << "X=" << SCALAR_AS_HEX(uint8_t, m_index_x)
							<< ", [" << SCALAR_AS_HEX(uint16_t, target) << "]="
								<< SCALAR_AS_HEX(uint8_t, bus.cpu_read(target))
							<< (boundary ? " (Boundary)" : "");
						break;
					case CPU_MODE_ABSOLUTE_Y:
						target = bus.cpu_read(program_counter++);
						target |= (bus.cpu_read(program_counter++) << CHAR_BIT);
						boundary = (((target & UINT8_MAX) + m_index_y) > UINT8_MAX);
						target += m_index_y;
					...
					case CPU_MODE_INDIRECT:
						...
						if((indirect & UINT8_MAX) == UINT8_MAX) {
							target = bus.cpu_read(indirect);
							target |= (bus.cpu_read(indirect - UINT8_MAX) << CHAR_BIT);
							wrap = true;
						} else {
							target = bus.cpu_read(indirect);
							target |= (bus.cpu_read(indirect + 1) << CHAR_BIT);
						}
					...
					case CPU_MODE_INDIRECT_X:
						indirect = bus.cpu_read(program_counter++);
						indirect += m_index_x;
						target = bus.cpu_read(indirect + 1);
						target |= (bus.cpu_read(indirect) << CHAR_BIT);
						stream << "X=" << SCALAR_AS_HEX(uint8_t, m_index_x)
					...
					case CPU_MODE_RELATIVE:
						...
						target = (program_counter + 1 + relative);
						boundary = (((program_counter++) & (UINT8_MAX << CHAR_BIT)) != ((target) & (UINT8_MAX << CHAR_BIT)));
						stream << "*=" << SCALAR_AS_HEX(uint16_t, target)
					...
					case CPU_MODE_ZERO_PAGE:
						...
						break;
					case CPU_MODE_ZERO_PAGE_X:
						target = (bus.cpu_read(program_counter++) + m_index_x);
						stream << "X=" << SCALAR_AS_HEX(uint8_t, m_index_x)
							<< ", [" << SCALAR_AS_HEX(uint8_t, target) << "]="
								<< SCALAR_AS_HEX(uint8_t, bus.cpu_read(target));
						break;
					case CPU_MODE_ZERO_PAGE_Y:
						target = (bus.cpu_read(program_counter++) + m_index_y);
						stream << "Y=" << SCALAR_AS_HEX(uint8_t, m_index_y)
							<< ", [" << SCALAR_AS_HEX(uint8_t, target) << "]="
								<< SCALAR_AS_HEX(uint8_t, bus.cpu_read(target));
						break;
				...
				if(verbose) {
					uint8_t iter_byte = 0, length = CPU_MODE_LENGTH(command.second);
					...
					for(; iter_byte < length; ++iter_byte) {
						...
						stream << SCALAR_AS_HEX(uint8_t, bus.cpu_read((program_counter - length) + iter_byte));
					}
			...
			)
		{
			...
			switch(command.second) {
				...
				case CPU_MODE_ZERO_PAGE_X:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_MODE,
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_BVS:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_BRANCH,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_CPY:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_COMPARE,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_DEY:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_DECREMENT,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_SEI:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_FLAG,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_ILLEGAL_SRE:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_COMMAND,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_ILLEGAL_XAA:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_LOAD,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.second) {
				...
				case CPU_MODE_ZERO_PAGE_X:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_NOP,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_ILLEGAL_TAS:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_STORE,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_INY:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_INCREMENT,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_PLP:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_STACK_PULL,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_PHP:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_STACK_PUSH,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			)
		{
			...
			switch(command.first) {
				...
				case CPU_COMMAND_TYA:
					...
					break;
				default:
					THROW_NESCC_EMULATOR_CPU_EXCEPTION_FORMAT(NESCC_EMULATOR_CPU_EXCEPTION_UNSUPPORTED_TRANSFER,
						"Address=%u(%04x), Command=%u(%02x), Mode=%u", m_program_counter, m_program_counter,
						command.first, command.first, command.second);
			}
			...
			) const
		{
			...
			result << NESCC_EMULATOR_CPU_HEADER << "(" << SCALAR_AS_HEX(uintptr_t, this) << ")";

			if(verbose) {
				result << " Base=" << nescc::core::singleton<nescc::emulator::cpu>::to_string(verbose);
