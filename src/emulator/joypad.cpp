...
#include "../../include/emulator/joypad.h"
...
namespace nescc {
	...
	namespace emulator {
...
#ifndef NDEBUG
			if(!m_initialized) {
				THROW_NESCC_EMULATOR_JOYPAD_EXCEPTION(NESCC_EMULATOR_JOYPAD_EXCEPTION_UNINITIALIZED);
			}
#endif // NDEBUG
			...
			if(SDL_IsGameController(event.which)) {
				...
				for(std::vector<std::pair<SDL_JoystickID, SDL_GameController *>>::iterator iter = m_controller.begin();
					...
					if(!iter->second) {
						...
						if(!iter->second) {
							THROW_NESCC_EMULATOR_JOYPAD_EXCEPTION_FORMAT(NESCC_EMULATOR_JOYPAD_EXCEPTION_EXTERNAL,
			...
			)
		{
			...
			if(pad > JOYPAD_MAX) {
				THROW_NESCC_EMULATOR_JOYPAD_EXCEPTION_FORMAT(NESCC_EMULATOR_JOYPAD_EXCEPTION_UNSUPPORTED,
			...
			) const
		{
			...
			result << NESCC_EMULATOR_JOYPAD_HEADER << "(" << SCALAR_AS_HEX(uintptr_t, this) << ")";

			if(verbose) {
				result << " Base=" << nescc::core::singleton<nescc::emulator::joypad>::to_string(verbose);
