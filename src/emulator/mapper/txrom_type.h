...
#ifndef NESCC_EMULATOR_MAPPER_TXROM_TYPE_H_
#define NESCC_EMULATOR_MAPPER_TXROM_TYPE_H_
...
namespace nescc {
	...
	namespace emulator {
		...
		namespace mapper {
			...
			#define NESCC_EMULATOR_MAPPER_TXROM_HEADER "[NESCC::EMULATOR::MAPPER::TXROM]"
#ifndef NDEBUG
			#define NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_HEADER NESCC_EMULATOR_MAPPER_TXROM_HEADER " "
#else
			#define NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_HEADER
#endif // NDEBUG

			enum {
				NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_UNSUPPORTED_ADDRESS = 0,
			};

			#define NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_MAX NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_UNSUPPORTED_ADDRESS

			static const std::string NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_STR[] = {
				NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_HEADER "Unsupported mapper address",
				};

			#define NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_STRING(_TYPE_) \
				(((_TYPE_) > NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_MAX) ? EXCEPTION_UNKNOWN : \
					STRING_CHECK(NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_STR[_TYPE_]))

			#define THROW_NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION(_EXCEPT_) \
				THROW_NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_FORMAT(_EXCEPT_, "", "")
			#define THROW_NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_FORMAT(_EXCEPT_, _FORMAT_, ...) \
				THROW_EXCEPTION_FORMAT(NESCC_EMULATOR_MAPPER_TXROM_EXCEPTION_STRING(_EXCEPT_), \
...
#endif // NESCC_EMULATOR_MAPPER_TXROM_TYPE_H_
