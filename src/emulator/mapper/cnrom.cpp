...
#include "../../../include/emulator/mapper/cnrom.h"
#include "../../../include/trace.h"
#include "./cnrom_type.h"
		...
		namespace mapper {
				...
				) const
			{
				...
				result << std::left << std::setw(COLUMN_WIDTH_LONG) << "Mirroring"
					...
					<< std::endl << std::left << std::setw(COLUMN_WIDTH_LONG) << "CHR ROM Data"
				...
				)
			{
				...
				m_rom_character_index = 0;
				m_rom_program_index_0 = 0;
				m_rom_program_index_1 = ((cartridge.rom_program_banks() > 1) ? 1 : 0); // mirror for CNROM-128
				...
				)
			{
				...
				switch(address) {
					...
					case MAPPER_PROGRAM_1_LOW ... MAPPER_PROGRAM_1_HIGH: // 0x4000 - 0x7fff
						...
						break;
					default:
						THROW_NESCC_EMULATOR_MAPPER_CNROM_EXCEPTION_FORMAT(
							NESCC_EMULATOR_MAPPER_CNROM_EXCEPTION_UNSUPPORTED_ADDRESS,
				...
				)
			{
				...
				TRACE_MESSAGE_FORMAT(TRACE_INFORMATION, "|-CHR ROM Data", "%u", m_port_select.raw);
				...
				) const
			{
				...
				result << NESCC_EMULATOR_MAPPER_CNROM_HEADER << "(" << SCALAR_AS_HEX(uintptr_t, this) << ")";
				...
				if(verbose) {
					result << ", PRG RAM Bank=" << (int) m_ram_index
						...
						<< ", CHR Bank Data=" << SCALAR_AS_HEX(uint8_t, m_port_select.select);
				}
				...
				)
			{
				...
				m_rom_character_index = m_port_select.select;
