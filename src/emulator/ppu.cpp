...
namespace nescc {
	...
	namespace emulator {
...
#ifndef NDEBUG
			if(!m_initialized) {
				THROW_NESCC_EMULATOR_PPU_EXCEPTION(NESCC_EMULATOR_PPU_EXCEPTION_UNINITIALIZED);
			}
#endif // NDEBUG
		...
		nescc::emulator::port_control_t
		ppu::control(void) const
		{
			nescc::emulator::port_control_t result;
			...
			)
		{
			...
			if(m_mask.sprite && !(!m_mask.sprite_left && (dot < PPU_SPRITE_LENGTH))) {
				std::vector<nescc::emulator::sprite_t>::reverse_iterator entry;
		...
		nescc::emulator::port_mask_t
		ppu::mask(void) const
		{
			nescc::emulator::port_mask_t result;
			...
			)
		{
			...
			switch(port) {
				...
				case PPU_PORT_DATA: // 0x2007
					...
					break;
				default:
					THROW_NESCC_EMULATOR_PPU_EXCEPTION_FORMAT(NESCC_EMULATOR_PPU_EXCEPTION_UNSUPPORTED,
			...
			)
		{
			...
			m_control = *((nescc::emulator::port_control_t *) &value);
			...
			)
		{
			...
			m_mask = *((nescc::emulator::port_mask_t *) &value);
			...
			)
		{
			...
			m_status = *((nescc::emulator::port_status_t *) &value);
		...
		ppu::sprite_evaluate(void)
		{
			...
			for(; iter < PPU_SPRITE_OAM_MAX; ++iter) {
				...
				if(((row >= 0) && (row < (m_control.sprite_size ? PPU_SPRITE_LENGTH_LONG : PPU_SPRITE_LENGTH)))) {
					nescc::emulator::sprite_t &entry = m_sprite_secondary.at(count);
			...
			)
		{
			...
			for(; iter < PPU_SPRITE_LENGTH; ++iter) {
				...
				nescc::emulator::sprite_t &entry = m_sprite.at(iter);
		...
		nescc::emulator::port_status_t
		ppu::status(void) const
		{
			nescc::emulator::port_status_t result;
			...
			) const
		{
			...
			result << NESCC_EMULATOR_PPU_HEADER << "(" << SCALAR_AS_HEX(uintptr_t, this) << ")";

			if(verbose) {
				result << " Base=" << nescc::core::singleton<nescc::emulator::ppu>::to_string(verbose);
